...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.adapter_linear_down = nn.Linear(dim, 384, bias=False)
        self.adapter_linear_up = nn.Linear(384, dim, bias=False)
        self.adapter_conv = nn.Conv3d(384, 384, kernel_size=(3, 1, 1), padding='same')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.contiguous().view(1, 20, 32, 32, 384)
        x = torch.permute(x, (0, -1, 1, 2, 3))
        x = self.adapter_conv(x)
        x = torch.permute(x, (0, 2, 3, 4, 1))
        x = x.contiguous().view(20, 32, 32, 384)
